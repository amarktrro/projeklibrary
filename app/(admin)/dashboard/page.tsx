  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-4xl font-black text-gray-800 tracking-tight">Dashboard Overview</h1>
          <p className="text-gray-600 font-bold mt-2">Monitoring data perpustakaan JTIK hari ini.</p>
        </div>
        <div className="bg-orange-900/20 text-orange-400 px-6 py-3 rounded-2xl border border-orange-700/30 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-[#172e5f] p-8 rounded-[2.5rem] shadow-sm border border-gray-300 flex flex-col gap-4 hover:shadow-xl hover:shadow-orange-500/10 transition-all group">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="text-[10px] font-black text-emerald-400 bg-emerald-900/30 px-2 py-0.5 rounded-full">{stat.trend}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[3rem] p-10 border border-gray-300 shadow-sm">
             <h3 className="text-xl font-black text-gray-800">Statistik Peminjaman</h3>
             <div className="h-64 mt-8 bg-gray-100 rounded-[2rem] border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 font-bold italic">
               Visualisasi data peminjaman bulanan
             </div>
        </div>

        {/* Aktivitas Terkini Section */}
        <div className="bg-white rounded-[3rem] p-10 shadow-xl flex flex-col border border-gray-300">
          <h3 className="text-2xl font-black mb-10 tracking-tight text-center text-gray-800">Aktivitas Terkini</h3>
          <div className="space-y-8 flex-1">
            {[
              ...
              <div key={i} className="flex gap-4 group cursor-default">
                ...
                <div>
                  <div className="font-black text-sm uppercase tracking-tight text-gray-800">{act.user}</div>
                  <div className="text-[10px] font-bold text-gray-500 mb-1">{act.time}</div>
                  <div className="text-sm italic text-gray-600 leading-relaxed">"{act.action}"</div>
                </div>
              </div>
            ))}
          </div>
          <button className="w-full mt-10 py-5 bg-orange-500/10 hover:bg-orange-500/20 border border-orange-300 rounded-2xl font-black text-xs uppercase tracking-widest transition-all text-orange-600">
